...
class SortableEdgeCases extends AnyFlatSpec with Matchers:
  ...
  "A correct swap" should "not fail" in {
    val list = SortableOld(0, 1, 5)
    list.swap(0, 2) match
      case Failure(_) => fail()
      case Success(l) => assert(l == SortableOld(List(5, 1, 0), List(Step.Swap(0, 2))))
  }
  ...
  "A correct comparison" should "not fail" in {
    ...
    list.compare(0, 1)(x => x)(x => x) match
      case Failure(_) => fail()
      case Success(l) => assert(l == SortableOld(List(0, 1, 5), List(Step.Comparison(0, 1))))
  }

  "Compare(0, 1) on an single element list" should "fail" in {
    val list = SortableOld(0)
    list.compare(0, 1)(x => x)(x => x) match
      case Failure(e) =>
      case Success(_) => fail()
  }
